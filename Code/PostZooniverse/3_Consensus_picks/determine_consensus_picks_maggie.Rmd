...
```{r}
MoreThan1<-species_votes %>% filter(votes > num_class)
```
...
```{r}
species_votes_to_check<- subjects_to_check %>% filter((propvote == propclass) & (between(propclass, 0.19, 0.79)))
```
...
```
Now make a version that includes the full metadata to which we will replace the choices once they are corrected

```{r}
subjects_to_fix<-Checklist
```
...
```{r}
write.csv(Checklist, here("Output", "PostZooniverse","files_to_check_28April2022.csv"))
```
...
```
Now join some columns from Flat

first extract them for simplicity
```{r}
meta<-Flat %>% select(classification_id,subject_ids,Phase, Lat, Long, Treatment, DTO)
```
Now summarise meta per subject id

```{r}
meta<-meta %>% group_by(subject_ids) %>% summarize(
  Phase = unique(Phase),
  Lat = unique(Lat),
  Long = unique(Long),
...
Now join them to assigned

```{r}
Assigned<-left_join(Assigned, meta)
```
And now join them to subject_to_fix
```{r}
subjects_to_fix<-left_join(subjects_to_fix, meta)
```
and save
```{r}
write.csv(subjects_to_fix,here("Output", "PostZooniverse","subjects_to_fix.csv"))
```
...
```{r}
write.csv(Assigned, here("Output", "PostZooniverse", "AssignedSpecies_28April2022.csv"))
```
